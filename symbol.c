...
void initSymbolTable (unsigned int size)
{
    ...
    hashTableSize = size;
    hashTable = (SymbolEntry **) newPtr(size * sizeof(SymbolEntry *));
...
void openScope ()
{
    Scope * newScope = (Scope *) newPtr(sizeof(Scope));
...
static SymbolEntry * newEntry (const char * name)
{
    ...
    e = (SymbolEntry *) newPtr(sizeof(SymbolEntry));
    e->id = (const char *) newPtr(strlen(name) + 1);
    ...
    switch (type->kind) {
        ...
        case TYPE_ARRAY:
            if (equalType(type->refType, typeChar)) {
                ...
                value.vString = (const char *) newPtr(strlen(str) + 1);
    ...
    switch (e->entryType) {
        ...
        case ENTRY_CONSTANT:
            if (e->u.eConstant.type->kind == TYPE_ARRAY)
                deletePtr((char *) (e->u.eConstant.value.vString));
